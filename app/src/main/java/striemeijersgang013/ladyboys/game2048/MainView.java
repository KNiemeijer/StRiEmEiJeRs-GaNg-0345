package striemeijersgang013.ladyboys.game2048;
...
public class MainView extends View {
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                ...
                if (currentTile != null) {
                    ...
                    ArrayList aArray = game.aGrid.getAnimationCell(xx, yy);
                    boolean animated = false;
                    for (int i = aArray.size() - 1; i >= 0; i--) {
                        AnimationCell aCell = (AnimationCell) aArray.get(i);
    ...
    private void getLayout(int width, int height) {
        ...
        startingX = (int) (screenMiddleX - (cellSize + gridWidth) * halfNumSquaresX - (float) gridWidth / 2);
        endingX = (int) (screenMiddleX + (cellSize + gridWidth) * halfNumSquaresX + (float) gridWidth / 2);
        startingY = (int) (boardMiddleY - (cellSize + gridWidth) * halfNumSquaresY - (float) gridWidth / 2);
        endingY = (int) (boardMiddleY + (cellSize + gridWidth) * halfNumSquaresY + (float) gridWidth / 2);
